...
const TestPage = props => {
  ...
  const {
    ...
    (typeFilter == "New Star Hit" && nshs.filter(nsh => nsh.node.year == dateFilter)[0].node) ||
    (typeFilter == "General Broadcasting" &&
      gbs.filter(gb => gb.node.year == dateFilter)[0].node) ||
    (typeFilter == "Interviews / Special" && lbs.filter(lb => lb.node.year == dateFilter)[0].node);
              ...
              <Row h="100vh">
                ...
                <Col size="5" d="flex" flexDir="row" align="center">
                  <Image
                    m={{ l: "-7vw" }}
                    w="115%"
                    src={filteredImg.node.childImageSharp.fluid.src}
                  />
                </Col>
              </Row>
            </>
